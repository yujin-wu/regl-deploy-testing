...
"}")}var k=e.gl,m=c.def(b,".location"),n=c.def(e.attributes,"[",m,"]");b=l.state;var v=l.buffer,w=[l.x,l.y,l.z,l.w],ka=["buffer","normalized","offset","stride"];1===b?f():2===b?h():(c("if(",b,"===",1,"){"),f(),c("}else{"),h(),c("}"))}var e=a.shared;l.forEach(function(l){var e=l.name,m=g.attributes[e],k;if(m){if(!f(m))return;k=m.append(a,c)}else{if(!f(R))return;var n=a.scopeAttrib(e);b.optional(function(){a.assert(c,n+".state","missing attribute "+e)});k={};y(new ha).forEach(function(a){k[a]=c.def(n,
".",a)})}T(a.link(l),h(l.info.type),k)})}function wd(a,c,g,l,f,h){for(var T=a.shared,e=T.gl,k,n=0;n<l.length;++n){var v=l[n],w=v.name,ka=v.info.type,x=g.uniforms[w],v=a.link(v)+".location",d;if(x){if(!f(x))continue;if(z(x)){var r=x.value;b.command(null!==r&&"undefined"!==typeof r,'missing uniform "'+w+'"',a.commandStr);if(35678===ka||35680===ka)b.command("function"===typeof r&&(35678===ka&&("texture2d"===r._reglType||"framebuffer"===r._reglType)||35680===ka&&("textureCube"===r._reglType||"framebufferCube"===
...
1;O.time=X();da();dc.procs.poll()}function e(){ba.refresh();da();dc.procs.refresh();K&&K.update()}function X(){return(c()-N)/1E3}console.log("REGL args are:",k);k=b(k);if(!k)return null;var pa=k.gl,Vb=pa.getContextAttributes(),Fb=pa.isContextLost(),ac=t(pa,k);if(!ac)return null;var ra=y(),Na=F(),Y=ac.extensions,K=Ca(pa,Y),N=c(),S=pa.drawingBufferWidth,na=pa.drawingBufferHeight,O={tick:0,time:0,viewportWidth:S,viewportHeight:na,framebufferWidth:S,framebufferHeight:na,drawingBufferWidth:S,drawingBufferHeight:na,
pixelRatio:k.pixelRatio},S={elements:null,primitive:4,count:-1,offset:0,instances:-1},Wb=ga(pa,Y),$a=za(pa,Na,k,function(a){return w.destroyBuffer(a)}),P=H(pa,Y,$a,Na),w=M(pa,Y,Wb,Na,$a,P,S),ia=r(pa,ra,Na,k),ba=J(pa,Y,Wb,function(){dc.procs.poll()},O,Na,k),Fa=u(pa,Y,Wb,Na,k),Oa=B(pa,Y,Wb,ba,Fa,Na),dc=oa(pa,ra,Y,Wb,$a,P,ba,Oa,{},w,ia,S,O,K,k),ra=I(pa,Oa,dc.procs.poll,O,Vb,Y,Wb),Ia=dc.next,W=pa.canvas,gb=[],ea=[],ta=[],ic=[k.onDestroy],pc=null;W&&(W.addEventListener("webglcontextlost",A,!1),W.addEventListener("webglcontextrestored",
C,!1));var sc=Oa.setFBO=m({framebuffer:z.define.call(null,1,"framebuffer")});e();Vb=q(m,{clear:function(a){V("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)sc(q({framebuffer:a.framebuffer.faces[b]},a),la);else sc(a,la);else la(null,a)},prop:z.define.bind(null,1),context:z.define.bind(null,2),"this":z.define.bind(null,3),draw:m({}),buffer:function(a){return $a.create(a,34962,!1,!1)},
elements:function(a){return P.create(a,!1)},texture:ba.create2D,cube:ba.createCube,renderbuffer:Fa.create,framebuffer:Oa.create,framebufferCube:Oa.createCube,vao:w.createVAO,attributes:Vb,frame:L,on:function(a,b){V.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return L(b);case "lost":c=ea;break;case "restore":c=ta;break;case "destroy":c=ic;break;default:V.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=
