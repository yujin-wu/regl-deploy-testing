...
1;M.time=U();ga();$b.procs.poll()}function g(){ba.refresh();ga();$b.procs.refresh();H&&H.update()}function U(){return(a()-L)/1E3}console.log("REGL args are:",q);q=d(q);console.log("REGL configs are:",q);if(!q)return null;var ka=q.gl,la=ka.getContextAttributes(),gb=ka.isContextLost(),hc=p(ka,q);if(!hc)return null;var sa=n(),Pa=N(),X=hc.extensions,H=Jc(ka,X),L=a(),Q=ka.drawingBufferWidth,qa=ka.drawingBufferHeight,M={tick:0,time:0,viewportWidth:Q,viewportHeight:qa,framebufferWidth:Q,framebufferHeight:qa,
drawingBufferWidth:Q,drawingBufferHeight:qa,pixelRatio:q.pixelRatio},Q={elements:null,primitive:4,count:-1,offset:0,instances:-1},Zb=aa(ka,X),$a=R(ka,Pa,q,function(a){return t.destroyBuffer(a)}),T=r(ka,X,$a,Pa),t=O(ka,X,Zb,Pa,$a,T,Q),ia=A(ka,sa,Pa,q),ba=wa(ka,X,Zb,function(){$b.procs.poll()},M,Pa,q),va=v(ka,X,Zb,Pa,q),La=C(ka,X,Zb,ba,va,Pa),$b=db(ka,sa,X,Zb,$a,T,ba,La,{},t,ia,Q,M,H,q),sa=I(ka,La,$b.procs.poll,M,la,X,Zb),Ua=$b.next,V=ka.canvas,pb=[],ca=[],ma=[],Rb=[q.onDestroy],oc=null;V&&(V.addEventListener("webglcontextlost",
P,!1),V.addEventListener("webglcontextrestored",K,!1));var tc=La.setFBO=w({framebuffer:b.define.call(null,1,"framebuffer")});g();la=c(w,{clear:function(a){l("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)tc(c({framebuffer:a.framebuffer.faces[b]},a),ea);else tc(a,ea);else ea(null,a)},prop:b.define.bind(null,1),context:b.define.bind(null,2),"this":b.define.bind(null,3),draw:w({}),buffer:function(a){return $a.create(a,
34962,!1,!1)},elements:function(a){return T.create(a,!1)},texture:ba.create2D,cube:ba.createCube,renderbuffer:va.create,framebuffer:La.create,framebufferCube:La.createCube,vao:t.createVAO,attributes:la,frame:G,on:function(a,b){l.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return G(b);case "lost":c=ca;break;case "restore":c=ma;break;case "destroy":c=Rb;break;default:l.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=
