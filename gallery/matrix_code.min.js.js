...
"}")}var h=k.gl,m=d.def(a,".location"),c=d.def(k.attributes,"[",m,"]");a=l.state;var v=l.buffer,z=[l.x,l.y,l.z,l.w],u=["buffer","normalized","offset","stride"];1===a?e():2===a?g():(d("if(",a,"===",1,"){"),e(),d("}else{"),g(),d("}"))}var k=b.shared;l.forEach(function(l){var k=l.name,m=f.attributes[k],h;if(m){if(!e(m))return;h=m.append(b,d)}else{if(!e(P))return;var c=b.scopeAttrib(k);a.optional(function(){b.assert(d,c+".state","missing attribute "+k)});h={};q(new ga).forEach(function(b){h[b]=d.def(c,
".",b)})}T(b.link(l),g(l.info.type),h)})}function ud(b,d,f,l,e,g){for(var T=b.shared,k=T.gl,h,c=0;c<l.length;++c){var v=l[c],z=v.name,u=v.info.type,ma=f.uniforms[z],v=b.link(v)+".location",w;if(ma){if(!e(ma))continue;if(A(ma)){var q=ma.value;a.command(null!==q&&"undefined"!==typeof q,'missing uniform "'+z+'"',b.commandStr);if(35678===u||35680===u)a.command("function"===typeof q&&(35678===u&&("texture2d"===q._reglType||"framebuffer"===q._reglType)||35680===u&&("textureCube"===q._reglType||"framebufferCube"===
...
sb[d]=b}}}function da(){var a=bb.viewport,b=bb.scissor_box;a[0]=a[1]=b[0]=b[1]=0;N.viewportWidth=N.framebufferWidth=N.drawingBufferWidth=a[2]=b[2]=la.drawingBufferWidth;N.viewportHeight=N.framebufferHeight=N.drawingBufferHeight=a[3]=b[3]=la.drawingBufferHeight}function b(){N.tick+=1;N.time=Y();da();ac.procs.poll()}function k(){ba.refresh();da();ac.procs.refresh();H&&H.update()}function Y(){return(d()-M)/1E3}console.log("REGL args are:",h);h=a(h);if(!h)return null;var la=h.gl,Ta=la.getContextAttributes(),
mb=la.isContextLost(),Eb=t(la,h);if(!Eb)return null;var qa=q(),Oa=O(),Z=Eb.extensions,H=Ca(la,Z),M=d(),R=la.drawingBufferWidth,ua=la.drawingBufferHeight,N={tick:0,time:0,viewportWidth:R,viewportHeight:ua,framebufferWidth:R,framebufferHeight:ua,drawingBufferWidth:R,drawingBufferHeight:ua,pixelRatio:h.pixelRatio},R={elements:null,primitive:4,count:-1,offset:0,instances:-1},Zb=ja(la,Z),Ja=wa(la,Oa,h,function(a){return u.destroyBuffer(a)}),Q=K(la,Z,Ja,Oa),u=F(la,Z,Zb,Oa,Ja,Q,R),ha=x(la,qa,Oa,h),ba=L(la,
Z,Zb,function(){ac.procs.poll()},N,Oa,h),Fa=r(la,Z,Zb,Oa,h),Ma=B(la,Z,Zb,ba,Fa,Oa),ac=ka(la,qa,Z,Zb,Ja,Q,ba,Ma,{},u,ha,R,N,H,h),qa=G(la,Ma,ac.procs.poll,N,Ta,Z,Zb),bb=ac.next,V=la.canvas,sb=[],ea=[],ra=[],ec=[h.onDestroy],pc=null;V&&(V.addEventListener("webglcontextlost",y,!1),V.addEventListener("webglcontextrestored",C,!1));var mc=Ma.setFBO=m({framebuffer:A.define.call(null,1,"framebuffer")});k();Ta=n(m,{clear:function(a){W("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in
a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)mc(n({framebuffer:a.framebuffer.faces[b]},a),na);else mc(a,na);else na(null,a)},prop:A.define.bind(null,1),context:A.define.bind(null,2),"this":A.define.bind(null,3),draw:m({}),buffer:function(a){return Ja.create(a,34962,!1,!1)},elements:function(a){return Q.create(a,!1)},texture:ba.create2D,cube:ba.createCube,renderbuffer:Fa.create,framebuffer:Ma.create,framebufferCube:Ma.createCube,vao:u.createVAO,attributes:Ta,frame:I,
