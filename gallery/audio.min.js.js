...
m,",",x,");",F.map(function(b,a){return c+"."+b+"="+x[a]+";"}).join(""),"}")}var h=k.gl,m=d.def(a,".location"),c=d.def(k.attributes,"[",m,"]");a=l.state;var n=l.buffer,x=[l.x,l.y,l.z,l.w],t=["buffer","normalized","offset","stride"];1===a?e():2===a?g():(d("if(",a,"===",1,"){"),e(),d("}else{"),g(),d("}"))}var k=b.shared;l.forEach(function(l){var k=l.name,m=f.attributes[k],h;if(m){if(!e(m))return;h=m.append(b,d)}else{if(!e(S))return;var c=b.scopeAttrib(k);a.optional(function(){b.assert(d,c+".state",
"missing attribute "+k)});h={};r(new ga).forEach(function(b){h[b]=d.def(c,".",b)})}O(b.link(l),g(l.info.type),h)})}function vd(b,d,f,l,e,g){for(var O=b.shared,k=O.gl,h,c=0;c<l.length;++c){var n=l[c],x=n.name,t=n.info.type,ka=f.uniforms[x],n=b.link(n)+".location",A;if(ka){if(!e(ka))continue;if(y(ka)){var r=ka.value;a.command(null!==r&&"undefined"!==typeof r,'missing uniform "'+x+'"',b.commandStr);if(35678===t||35680===t)a.command("function"===typeof r&&(35678===t&&("texture2d"===r._reglType||"framebuffer"===
...
J.drawingBufferHeight=a[3]=b[3]=pa.drawingBufferHeight}function b(){J.tick+=1;J.time=X();da();ac.procs.poll()}function k(){Z.refresh();da();ac.procs.refresh();N&&N.update()}function X(){return(d()-M)/1E3}console.log("REGL args are:",h);h=a(h);if(!h)return null;var pa=h.gl,Va=pa.getContextAttributes(),jb=pa.isContextLost(),Gb=u(pa,h);if(!Gb)return null;var ua=r(),Na=P(),Y=Gb.extensions,N=va(pa,Y),M=d(),T=pa.drawingBufferWidth,qa=pa.drawingBufferHeight,J={tick:0,time:0,viewportWidth:T,viewportHeight:qa,
framebufferWidth:T,framebufferHeight:qa,drawingBufferWidth:T,drawingBufferHeight:qa,pixelRatio:h.pixelRatio},T={elements:null,primitive:4,count:-1,offset:0,instances:-1},Zb=ja(pa,Y),Ja=sa(pa,Na,h,function(a){return t.destroyBuffer(a)}),R=n(pa,Y,Ja,Na),t=L(pa,Y,Zb,Na,Ja,R,T),ha=C(pa,ua,Na,h),Z=I(pa,Y,Zb,function(){ac.procs.poll()},J,Na,h),Aa=v(pa,Y,Zb,Na,h),Oa=D(pa,Y,Zb,Z,Aa,Na),ac=na(pa,ua,Y,Zb,Ja,R,Z,Oa,{},t,ha,T,J,N,h),ua=F(pa,Oa,ac.procs.poll,J,Va,Y,Zb),bb=ac.next,U=pa.canvas,ob=[],ca=[],oa=[],
fc=[h.onDestroy],gc=null;U&&(U.addEventListener("webglcontextlost",B,!1),U.addEventListener("webglcontextrestored",aa,!1));var rc=Oa.setFBO=m({framebuffer:y.define.call(null,1,"framebuffer")});k();Va=p(m,{clear:function(a){Q("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)rc(p({framebuffer:a.framebuffer.faces[b]},a),la);else rc(a,la);else la(null,a)},prop:y.define.bind(null,1),context:y.define.bind(null,
2),"this":y.define.bind(null,3),draw:m({}),buffer:function(a){return Ja.create(a,34962,!1,!1)},elements:function(a){return R.create(a,!1)},texture:Z.create2D,cube:Z.createCube,renderbuffer:Aa.create,framebuffer:Oa.create,framebufferCube:Oa.createCube,vao:t.createVAO,attributes:Va,frame:H,on:function(a,b){Q.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return H(b);case "lost":c=ca;break;case "restore":c=oa;break;case "destroy":c=fc;break;default:Q.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);
return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();break}}}},limits:Zb,hasExtension:function(a){return 0<=Zb.extensions.indexOf(a.toLowerCase())},read:ua,destroy:function(){ob.length=0;w();U&&(U.removeEventListener("webglcontextlost",B),U.removeEventListener("webglcontextrestored",aa));ha.clear();Oa.clear();Aa.clear();t.clear();Z.clear();R.clear();Ja.clear();N&&N.clear();fc.forEach(function(a){a()})},_gl:pa,_refresh:k,poll:function(){b();N&&N.update()},now:X,
stats:Na});h.onDone(null,Va);return Va}},{"./lib/attribute":2,"./lib/buffer":3,"./lib/core":8,"./lib/dynamic":9,"./lib/elements":10,"./lib/extension":11,"./lib/framebuffer":12,"./lib/limits":13,"./lib/read":14,"./lib/renderbuffer":15,"./lib/shader":16,"./lib/stats":17,"./lib/strings":18,"./lib/texture":19,"./lib/timer":20,"./lib/util/check":21,"./lib/util/clock":22,"./lib/util/extend":25,"./lib/util/raf":33,"./lib/webgl":37}]},{},[1]);
