...
O)/1E3}console.log("REGL args are:",d);d=a(d);console.log("REGL configs are:",d);if(!d)return null;var la=d.gl,Ha=la.getContextAttributes(),gb=la.isContextLost(),wc=x(la,d);if(!wc)return null;var ma=t(),Na=L(),Z=wc.extensions,K=Wd(la,Z),O=b(),U=la.drawingBufferWidth,qa=la.drawingBufferHeight,G={tick:0,time:0,viewportWidth:U,viewportHeight:qa,framebufferWidth:U,framebufferHeight:qa,drawingBufferWidth:U,drawingBufferHeight:qa,pixelRatio:d.pixelRatio},U={elements:null,primitive:4,count:-1,offset:0,instances:-1},
Zb=pa(la,Z),$a=fa(la,Na,d,function(a){return v.destroyBuffer(a)}),Q=M(la,Z,$a,Na),v=A(la,Z,Zb,Na,$a,Q,U),ja=B(la,ma,Na,d),ca=H(la,Z,Zb,function(){dc.procs.poll()},G,Na,d),Ea=r(la,Z,Zb,Na,d),Oa=C(la,Z,Zb,ca,Ea,Na),dc=R(la,ma,Z,Zb,$a,Q,ca,Oa,{},v,ja,U,G,K,d),ma=P(la,Oa,dc.procs.poll,G,Ha,Z,Zb),Ia=dc.next,W=la.canvas,sb=[],ha=[],ua=[],Wb=[d.onDestroy],nc=null;W&&(W.addEventListener("webglcontextlost",z,!1),W.addEventListener("webglcontextrestored",T,!1));var jc=Oa.setFBO=q({framebuffer:N.define.call(null,
1,"framebuffer")});k();Ha=m(q,{clear:function(a){y("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)jc(m({framebuffer:a.framebuffer.faces[b]},a),oa);else jc(a,oa);else oa(null,a)},prop:N.define.bind(null,1),context:N.define.bind(null,2),"this":N.define.bind(null,3),draw:q({}),buffer:function(a){return $a.create(a,34962,!1,!1)},elements:function(a){return Q.create(a,!1)},texture:ca.create2D,
cube:ca.createCube,renderbuffer:Ea.create,framebuffer:Oa.create,framebufferCube:Oa.createCube,vao:v.createVAO,attributes:Ha,frame:I,on:function(a,b){y.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return I(b);case "lost":c=ha;break;case "restore":c=ua;break;case "destroy":c=Wb;break;default:y.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();
break}}}},limits:Zb,hasExtension:function(a){return 0<=Zb.extensions.indexOf(a.toLowerCase())},read:ma,destroy:function(){sb.length=0;n();W&&(W.removeEventListener("webglcontextlost",z),W.removeEventListener("webglcontextrestored",T));ja.clear();Oa.clear();Ea.clear();v.clear();ca.clear();Q.clear();$a.clear();K&&K.clear();Wb.forEach(function(a){a()})},_gl:la,_refresh:k,poll:function(){c();K&&K.update()},now:ba,stats:Na});d.onDone(null,Ha);return Ha}},{"./lib/attribute":2,"./lib/buffer":3,"./lib/core":8,
