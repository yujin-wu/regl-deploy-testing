...
pb[d]=b}}}function da(){var a=Ha.viewport,b=Ha.scissor_box;a[0]=a[1]=b[0]=b[1]=0;J.viewportWidth=J.framebufferWidth=J.drawingBufferWidth=a[2]=b[2]=ma.drawingBufferWidth;J.viewportHeight=J.framebufferHeight=J.drawingBufferHeight=a[3]=b[3]=ma.drawingBufferHeight}function a(){J.tick+=1;J.time=W();da();vb.procs.poll()}function n(){ba.refresh();da();vb.procs.refresh();L&&L.update()}function W(){return(d()-O)/1E3}console.log("REGL args are:",h);h=b(h);console.log("REGL configs are:",h);if(!h)return null;
var ma=h.gl,Ka=ma.getContextAttributes(),db=ma.isContextLost(),Za=w(ma,h);if(!Za)return null;var la=z(),Qa=N(),Z=Za.extensions,L=sd(ma,Z),O=d(),U=ma.drawingBufferWidth,ra=ma.drawingBufferHeight,J={tick:0,time:0,viewportWidth:U,viewportHeight:ra,framebufferWidth:U,framebufferHeight:ra,drawingBufferWidth:U,drawingBufferHeight:ra,pixelRatio:h.pixelRatio},U={elements:null,primitive:4,count:-1,offset:0,instances:-1},Wb=na(ma,Z),ab=fa(ma,Qa,h,function(a){return u.destroyBuffer(a)}),R=M(ma,Z,ab,Qa),u=C(ma,
Z,Wb,Qa,ab,R,U),ka=E(ma,la,Qa,h),ba=H(ma,Z,Wb,function(){vb.procs.poll()},J,Qa,h),Ea=r(ma,Z,Wb,Qa,h),Na=D(ma,Z,Wb,ba,Ea,Qa),vb=A(ma,la,Z,Wb,ab,R,ba,Na,{},u,ka,U,J,L,h),la=Q(ma,Na,vb.procs.poll,J,Ka,Z,Wb),Ha=vb.next,V=ma.canvas,pb=[],ea=[],va=[],ac=[h.onDestroy],pc=null;V&&(V.addEventListener("webglcontextlost",B,!1),V.addEventListener("webglcontextrestored",Y,!1));var ic=Na.setFBO=m({framebuffer:P.define.call(null,1,"framebuffer")});n();Ka=p(m,{clear:function(a){y("object"===typeof a&&a,"regl.clear() takes an object as input");
...
attributes:Ka,frame:K,on:function(a,b){y.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return K(b);case "lost":c=ea;break;case "restore":c=va;break;case "destroy":c=ac;break;default:y.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();break}}}},limits:Wb,hasExtension:function(a){return 0<=Wb.extensions.indexOf(a.toLowerCase())},read:la,destroy:function(){pb.length=
