...
rb[c]=b}}}function fa(){var a=Ga.viewport,b=Ga.scissor_box;a[0]=a[1]=b[0]=b[1]=0;N.viewportWidth=N.framebufferWidth=N.drawingBufferWidth=a[2]=b[2]=Ka.drawingBufferWidth;N.viewportHeight=N.framebufferHeight=N.drawingBufferHeight=a[3]=b[3]=Ka.drawingBufferHeight}function b(){N.tick+=1;N.time=Z();fa();fc.procs.poll()}function e(){ca.refresh();fa();fc.procs.refresh();K&&K.update()}function Z(){return(c()-L)/1E3}console.log("REGL args are:",h);h=a(h);if(!h)return null;var Ka=h.gl,Ra=Ka.getContextAttributes(),
zb=Ka.isContextLost(),Xb=w(Ka,h);if(!Xb)return null;var na=x(),Qa=Q(),X=Xb.extensions,K=Ia(Ka,X),L=c(),T=Ka.drawingBufferWidth,ma=Ka.drawingBufferHeight,N={tick:0,time:0,viewportWidth:T,viewportHeight:ma,framebufferWidth:T,framebufferHeight:ma,drawingBufferWidth:T,drawingBufferHeight:ma,pixelRatio:h.pixelRatio},T={elements:null,primitive:4,count:-1,offset:0,instances:-1},Rb=la(Ka,X),Za=W(Ka,Qa,h,function(a){return u.destroyBuffer(a)}),S=O(Ka,X,Za,Qa),u=z(Ka,X,Rb,Qa,Za,S,T),ka=B(Ka,na,Qa,h),ca=F(Ka,
X,Rb,function(){fc.procs.poll()},N,Qa,h),za=r(Ka,X,Rb,Qa,h),Na=D(Ka,X,Rb,ca,za,Qa),fc=m(Ka,na,X,Rb,Za,S,ca,Na,{},u,ka,T,N,K,h),na=H(Ka,Na,fc.procs.poll,N,Ra,X,Rb),Ga=fc.next,U=Ka.canvas,rb=[],da=[],ra=[],Yb=[h.onDestroy],dc=null;U&&(U.addEventListener("webglcontextlost",C,!1),U.addEventListener("webglcontextrestored",aa,!1));var qc=Na.setFBO=n({framebuffer:I.define.call(null,1,"framebuffer")});e();Ra=q(n,{clear:function(a){y("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in
a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)qc(q({framebuffer:a.framebuffer.faces[b]},a),pa);else qc(a,pa);else pa(null,a)},prop:I.define.bind(null,1),context:I.define.bind(null,2),"this":I.define.bind(null,3),draw:n({}),buffer:function(a){return Za.create(a,34962,!1,!1)},elements:function(a){return S.create(a,!1)},texture:ca.create2D,cube:ca.createCube,renderbuffer:za.create,framebuffer:Na.create,framebufferCube:Na.createCube,vao:u.createVAO,attributes:Ra,frame:M,
on:function(a,b){y.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return M(b);case "lost":c=da;break;case "restore":c=ra;break;case "destroy":c=Yb;break;default:y.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();break}}}},limits:Rb,hasExtension:function(a){return 0<=Rb.extensions.indexOf(a.toLowerCase())},read:na,destroy:function(){rb.length=
