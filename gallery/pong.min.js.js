...
N)/1E3}console.log("REGL args are:",m);m=c(m);console.log("REGL configs are:",m);if(!m)return null;var qa=m.gl,fb=qa.getContextAttributes(),gb=qa.isContextLost(),Rb=x(qa,m);if(!Rb)return null;var ta=sa(),Aa=O(),Z=Rb.extensions,K=Zb(qa,Z),N=b(),R=qa.drawingBufferWidth,oa=qa.drawingBufferHeight,P={tick:0,time:0,viewportWidth:R,viewportHeight:oa,framebufferWidth:R,framebufferHeight:oa,drawingBufferWidth:R,drawingBufferHeight:oa,pixelRatio:m.pixelRatio},R={elements:null,primitive:4,count:-1,offset:0,
instances:-1},Jb=jc(qa,Z),Za=ea(qa,Aa,m,function(a){return r.destroyBuffer(a)}),T=H(qa,Z,Za,Aa),r=C(qa,Z,Jb,Aa,Za,T,R),ja=B(qa,ta,Aa,m),ba=I(qa,Z,Jb,function(){dc.procs.poll()},P,Aa,m),Ha=y(qa,Z,Jb,Aa,m),Ja=A(qa,Z,Jb,ba,Ha,Aa),dc=z(qa,ta,Z,Jb,Za,T,ba,Ja,{},r,ja,R,P,K,m),ta=J(qa,Ja,dc.procs.poll,P,fb,Z,Jb),Xa=dc.next,W=qa.canvas,sb=[],ga=[],va=[],Xb=[m.onDestroy],qc=null;W&&(W.addEventListener("webglcontextlost",M,!1),W.addEventListener("webglcontextrestored",U,!1));var sc=Ja.setFBO=n({framebuffer:w.define.call(null,
1,"framebuffer")});h();fb=k(n,{clear:function(a){d("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)sc(k({framebuffer:a.framebuffer.faces[b]},a),la);else sc(a,la);else la(null,a)},prop:w.define.bind(null,1),context:w.define.bind(null,2),"this":w.define.bind(null,3),draw:n({}),buffer:function(a){return Za.create(a,34962,!1,!1)},elements:function(a){return T.create(a,!1)},texture:ba.create2D,
cube:ba.createCube,renderbuffer:Ha.create,framebuffer:Ja.create,framebufferCube:Ja.createCube,vao:r.createVAO,attributes:fb,frame:Q,on:function(a,b){d.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return Q(b);case "lost":c=ga;break;case "restore":c=va;break;case "destroy":c=Xb;break;default:d.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();
break}}}},limits:Jb,hasExtension:function(a){return 0<=Jb.extensions.indexOf(a.toLowerCase())},read:ta,destroy:function(){sb.length=0;t();W&&(W.removeEventListener("webglcontextlost",M),W.removeEventListener("webglcontextrestored",U));ja.clear();Ja.clear();Ha.clear();r.clear();ba.clear();T.clear();Za.clear();K&&K.clear();Xb.forEach(function(a){a()})},_gl:qa,_refresh:h,poll:function(){e();K&&K.update()},now:Y,stats:Aa});m.onDone(null,fb);return fb}},{"./lib/attribute":2,"./lib/buffer":3,"./lib/core":8,
