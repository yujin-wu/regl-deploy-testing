...
M)/1E3}console.log("REGL args are:",k);k=a(k);if(!k)return null;var sa=k.gl,Ka=sa.getContextAttributes(),Ja=sa.isContextLost(),Zb=g(sa,k);if(!Zb)return null;var ua=p(),Oa=R(),X=Zb.extensions,K=mc(sa,X),M=b(),Q=sa.drawingBufferWidth,oa=sa.drawingBufferHeight,L={tick:0,time:0,viewportWidth:Q,viewportHeight:oa,framebufferWidth:Q,framebufferHeight:oa,drawingBufferWidth:Q,drawingBufferHeight:oa,pixelRatio:k.pixelRatio},Q={elements:null,primitive:4,count:-1,offset:0,instances:-1},ac=r(sa,X),$a=x(sa,Oa,
k,function(a){return v.destroyBuffer(a)}),T=f(sa,X,$a,Oa),v=G(sa,X,ac,Oa,$a,T,Q),ja=C(sa,ua,Oa,k),ba=Z(sa,X,ac,function(){cc.procs.poll()},L,Oa,k),Ba=w(sa,X,ac,Oa,k),Ha=D(sa,X,ac,ba,Ba,Oa),cc=Mc(sa,ua,X,ac,$a,T,ba,Ha,{},v,ja,Q,L,K,k),ua=O(sa,Ha,cc.procs.poll,L,Ka,X,ac),Ga=cc.next,U=sa.canvas,pb=[],da=[],ra=[],Ib=[k.onDestroy],pc=null;U&&(U.addEventListener("webglcontextlost",P,!1),U.addEventListener("webglcontextrestored",S,!1));var yc=Ha.setFBO=t({framebuffer:I.define.call(null,1,"framebuffer")});
m();Ka=n(t,{clear:function(a){ga("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)yc(n({framebuffer:a.framebuffer.faces[b]},a),la);else yc(a,la);else la(null,a)},prop:I.define.bind(null,1),context:I.define.bind(null,2),"this":I.define.bind(null,3),draw:t({}),buffer:function(a){return $a.create(a,34962,!1,!1)},elements:function(a){return T.create(a,!1)},texture:ba.create2D,cube:ba.createCube,
renderbuffer:Ba.create,framebuffer:Ha.create,framebufferCube:Ha.createCube,vao:v.createVAO,attributes:Ka,frame:N,on:function(a,b){ga.type(b,"function","listener callback must be a function");var c;switch(a){case "frame":return N(b);case "lost":c=da;break;case "restore":c=ra;break;case "destroy":c=Ib;break;default:ga.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();break}}}},limits:ac,
hasExtension:function(a){return 0<=ac.extensions.indexOf(a.toLowerCase())},read:ua,destroy:function(){pb.length=0;B();U&&(U.removeEventListener("webglcontextlost",P),U.removeEventListener("webglcontextrestored",S));ja.clear();Ha.clear();Ba.clear();v.clear();ba.clear();T.clear();$a.clear();K&&K.clear();Ib.forEach(function(a){a()})},_gl:sa,_refresh:m,poll:function(){d();K&&K.update()},now:Y,stats:Oa});k.onDone(null,Ka);return Ka}},{"./lib/attribute":2,"./lib/buffer":3,"./lib/core":8,"./lib/dynamic":9,
