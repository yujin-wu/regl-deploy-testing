...
Ka.viewport,b=Ka.scissor_box;a[0]=a[1]=b[0]=b[1]=0;S.viewportWidth=S.framebufferWidth=S.drawingBufferWidth=a[2]=b[2]=la.drawingBufferWidth;S.viewportHeight=S.framebufferHeight=S.drawingBufferHeight=a[3]=b[3]=la.drawingBufferHeight}function I(){S.tick+=1;S.time=J();ba();Qa.procs.poll()}function r(){Z.refresh();ba();Qa.procs.refresh();ga&&ga.update()}function J(){return(d()-O)/1E3}console.log("REGL args are:",a);a=n(a);if(!a)return null;var la=a.gl,ia=la.getContextAttributes(),ua=la.isContextLost(),
ab=v(la,a);if(!ab)return null;var ma=D(),za=A(),da=ab.extensions,ga=aa(la,da),O=d(),T=la.drawingBufferWidth,ra=la.drawingBufferHeight,S={tick:0,time:0,viewportWidth:T,viewportHeight:ra,framebufferWidth:T,framebufferHeight:ra,drawingBufferWidth:T,drawingBufferHeight:ra,pixelRatio:a.pixelRatio},T={elements:null,primitive:4,count:-1,offset:0,instances:-1},Gb=z(la,da),Ta=w(la,za,a,function(a){return E.destroyBuffer(a)}),X=p(la,da,Ta,za),E=x(la,da,Gb,za,Ta,X,T),ka=y(la,ma,za,a),Z=e(la,da,Gb,function(){Qa.procs.poll()},
S,za,a),sa=c(la,da,Gb,za,a),wa=m(la,da,Gb,Z,sa,za),Qa=na(la,ma,da,Gb,Ta,X,Z,wa,{},E,ka,T,S,ga,a),ma=H(la,wa,Qa.procs.poll,S,ia,da,Gb),Ka=Qa.next,U=la.canvas,Na=[],ea=[],pa=[],Xa=[a.onDestroy],Cb=null;U&&(U.addEventListener("webglcontextlost",L,!1),U.addEventListener("webglcontextrestored",W,!1));var yb=wa.setFBO=F({framebuffer:q.define.call(null,1,"framebuffer")});r();ia=g(F,{clear:function(a){k("object"===typeof a&&a,"regl.clear() takes an object as input");if("framebuffer"in a)if(a.framebuffer&&
"framebufferCube"===a.framebuffer_reglType)for(var b=0;6>b;++b)yb(g({framebuffer:a.framebuffer.faces[b]},a),fa);else yb(a,fa);else fa(null,a)},prop:q.define.bind(null,1),context:q.define.bind(null,2),"this":q.define.bind(null,3),draw:F({}),buffer:function(a){return Ta.create(a,34962,!1,!1)},elements:function(a){return X.create(a,!1)},texture:Z.create2D,cube:Z.createCube,renderbuffer:sa.create,framebuffer:wa.create,framebufferCube:wa.createCube,vao:E.createVAO,attributes:ia,frame:N,on:function(a,b){k.type(b,
"function","listener callback must be a function");var c;switch(a){case "frame":return N(b);case "lost":c=ea;break;case "restore":c=pa;break;case "destroy":c=Xa;break;default:k.raise("invalid event, must be one of frame,lost,restore,destroy")}c.push(b);return{cancel:function(){for(var a=0;a<c.length;++a)if(c[a]===b){c[a]=c[c.length-1];c.pop();break}}}},limits:Gb,hasExtension:function(a){return 0<=Gb.extensions.indexOf(a.toLowerCase())},read:ma,destroy:function(){Na.length=0;u();U&&(U.removeEventListener("webglcontextlost",
...
"./lib/strings":18,"./lib/texture":19,"./lib/timer":20,"./lib/util/check":21,"./lib/util/clock":22,"./lib/util/extend":25,"./lib/util/raf":33,"./lib/webgl":37}]},{},[1]);
